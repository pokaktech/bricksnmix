...
# from .models import Order, OrderDetails
from .models import CustomerOrder, OrderItem, OrderProductImage


def orders_cart_obj(request):
        try:
            if request.user.is_authenticated and not request.user.is_anonymous:
                cart = CustomerOrder.objects.all().filter(user = request.user, is_finished=False).first()
            else:    
                cart_id = request.session.get('cart_id')                     
                cart = CustomerOrder.objects.all().filter(id = cart_id, is_finished=False)
            
        except:
                cart =False  
        if cart:
            if request.user.is_authenticated and not request.user.is_anonymous:
                order_context = CustomerOrder.objects.filter(
                    user = request.user, is_finished=False).first()
            else:    
                order_context = CustomerOrder.objects.get(
                    id = cart_id, is_finished=False)
            order_details_context = OrderItem.objects.all().filter(order=order_context)
            cart_count = OrderItem.objects.all().filter(order=order_context).count()
