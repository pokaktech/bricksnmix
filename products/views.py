...
def product_details_api(request, slug):
    print(f"Received slug: {slug}")  # Debug print
    product_detail = get_object_or_404(Product, PRDSlug=slug, PRDISactive=True)
    serializer = ProductSerializer(product_detail)

    product_feedback = ProductRating.objects.filter(PRDIProduct=product_detail, active=True)
    feedback_sum = product_feedback.aggregate(Sum('rate'))
    feedbak_number = product_feedback.count()

    try:
        average_rating = int(feedback_sum["rate__sum"]) / feedbak_number

        start_1_sum = product_feedback.filter(rate=1).count()
        start_2_sum = product_feedback.filter(rate=2).count()
        start_3_sum = product_feedback.filter(rate=3).count()
        start_4_sum = product_feedback.filter(rate=4).count()
        start_5_sum = product_feedback.filter(rate=5).count()
        ...
        average_rating = 0
        start_1 = start_2 = start_3 = start_4 = start_5 = 0
